...
#include "../objects/OracleDBAUser.h"
#include "../objects/OracleUserTables.h"
#include "../parsers/OracleTags.h"
#include "../objects/OracleDBATables.h"
#include "../objects/OracleTableColumn.h"

#include "../indices/OracleDBATablesColumns.h"
#include "../dsl/OracleObject.h"
#include "../dsl/OracleSchemaObject.h"
#include "../indices/OracleColsColumns.h"
#include "../indices/OracleDbaColsColumns.h"
#include "../objects/OracleTableColsDba.h"
            ...
            public:
                ...
                std::vector<OracleDBAUser> getAllDBAUsers() {
                    ...
                    while (resultSet->next()) {
                        ...
                        std::string default_tablespace = resultSet->getString(
                                OracleDBAUsersColumns::DEFAULT_TABLESPACE::INDEX);
                        std::string temporary_tablespace = resultSet->getString(
                                OracleDBAUsersColumns::TEMPORARY_TABLESPACE::INDEX);
                        ...
                        std::string password_versions = resultSet->getString(
                                OracleDBAUsersColumns::PASSWORD_VERSIONS::INDEX);
                        std::string editions_enabled = resultSet->getString(
                                OracleDBAUsersColumns::EDITIONS_ENABLED::INDEX);
                        std::string authentication_type = resultSet->getString(
                                OracleDBAUsersColumns::AUTHENTICATION_TYPE::INDEX);
                ...
                std::vector<OracleUserTables> getOracleUsersTables() {
                    ...
                    while (resultSet->next()) {
                        ...
                        std::string TABLESPACE_NAME = resultSet->getString(
                                OracleAllTablesColumns::TABLESPACE_NAME::INDEX);
                        ...
                        std::string INITIAL_EXTENT = resultSet->getString(
                                OracleAllTablesColumns::INITIAL_EXTENT::INDEX);
                        ...
                        std::string FREELIST_GROUPS = resultSet->getString(
                                OracleAllTablesColumns::FREELIST_GROUPS::INDEX);
                        ...
                        std::string NUM_FREELIST_BLOCKS = resultSet->getString(
                                OracleAllTablesColumns::NUM_FREELIST_BLOCKS::INDEX);
                        ...
                        std::string CELL_FLASH_CACHE = resultSet->getString(
                                OracleAllTablesColumns::CELL_FLASH_CACHE::INDEX);
                        ...
                        std::string SEGMENT_CREATED = resultSet->getString(
                                OracleAllTablesColumns::SEGMENT_CREATED::INDEX);
                        ...
                        userTables.emplace_back(OracleUserTables{OWNER, TABLE_NAME, TABLESPACE_NAME,
                                                                 ...
                                                                 CELL_FLASH_CACHE, ROW_MOVEMENT, GLOBAL_STATS,
                                                                 USER_STATS,
                ...
                std::vector<OracleDBATables> getDBATables() {
                    ...
                    while (resultSet->next()) {
                        ...
                        std::string tablespace_name = resultSet->getString(
                                OracleDBATablesColumns::TABLESPACE_NAME::INDEX);
                        ...
                        int avg_space_freelist_blocks = resultSet->getInt(
                                OracleDBATablesColumns::AVG_SPACE_FREELIST_BLOCKS::INDEX);
                        ...
                        std::string cell_flash_cache = resultSet->getString(
                                OracleDBATablesColumns::CELL_FLASH_CACHE::INDEX);
                        ...
                        std::string segment_created = resultSet->getString(
                                OracleDBATablesColumns::SEGMENT_CREATED::INDEX);
                        ...
                        dbaTables.emplace_back(OracleDBATables{
                                ...
                                pct_increase,
                                freelists, freelist_groups, logging, backed_up, num_rows, blocks, empty_blocks,
                                avg_space,
                                chain_cnt, avg_row_len, avg_space_freelist_blocks, num_freelist_blocks, degree,
                                instances, cache,
                                table_lock, sample_size, last_analyzed, partitioned, iot_type, temporary, secondary,
                                nested,
                                buffer_pool, flash_cache, cell_flash_cache, row_movement, global_stats, user_stats,
                                duration,
                                skip_corrupt, monitoring, cluster_owner, dependencies, compression, compress_for,
                                dropped,
                                read_only, segment_created, result_cache});
                    }
                 ...
                 */
                std::string
                getDDL(const std::string &oracleObject, const std::string &schema, const std::string &objectName) {
                ...
                std::vector<OracleTableColumn> getTableColumns(const std::string &tableName) {
                    ...
                    while (resultSet->next()) {
                        ...
                        std::string character_set_name = resultSet->getString(
                                OracleColsColumns::CHARACTER_SET_NAME::INDEX);
                        std::string char_col_decl_length = resultSet->getString(
                                OracleColsColumns::CHAR_COL_DECL_LENGTH::INDEX);
                        ...
                        tableColumns.emplace_back(OracleTableColumn{table_name, column_name, data_type,
                                                                    ...
                                                                    column_id, default_length, data_default,
                                                                    num_distinct,
                                                                    ...
                                                                    avg_col_len,
                                                                    char_length, char_used, v80_fmt_image,
                ...
                std::vector<OracleTAbleColsDba> getTableColumnsDba(const std::string &tableName) {
                    ...
                    while (resultSet->next()) {
                        ...
                        std::string data_type_owner = resultSet->getString(
                                OracleDbaColsColumns::DATA_TYPE_OWNER::INDEX);
                        ...
                        std::string character_set_name = resultSet->getString(
                                OracleDbaColsColumns::CHARACTER_SET_NAME::INDEX);
                        std::string char_col_decl_length = resultSet->getString(
                                OracleDbaColsColumns::CHAR_COL_DECL_LENGTH::INDEX);
                        ...
                        std::string qualified_col_name = resultSet->getString(
                                OracleDbaColsColumns::QUALIFIED_COL_NAME::INDEX);
