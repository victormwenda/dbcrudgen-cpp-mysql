...
:occidml
set filename=%1
cl -GX -DWIN32COMMON -I. -I%ICINCHOME% -I. -D_CRT_SECURE_NO_DEPRECATE -D_DLL -D_MT %1.cpp /link /LIBPATH:%ICLIBHOME% oci.lib msvcrt.lib msvcprt.lib oraocci18.lib /nod:libc %BUFFEROVERFLOWLIB%
if exist %filename%.exe.manifest call %MTBIN% -manifest %filename%.exe.manifest -outputresource:%filename%.exe;#1 & %SRCHOME%/buildtools/bin/ntrmln.bat rmf %filename%.exe.manifest
goto end
...
cl -GX -DWIN32COMMON -I. -I%ICINCHOME% -I. -D_CRT_SECURE_NO_DEPRECATE -D_DLL -D_MT %1.cpp %1o.cpp %1m.cpp /link /LIBPATH:%ICLIBHOME% oci.lib msvcrt.lib msvcprt.lib oraocci18.lib /nod:libc %BUFFEROVERFLOWLIB%
if exist %filename%.exe.manifest call %MTBIN% -manifest %filename%.exe.manifest -outputresource:%filename%.exe;#1 & %SRCHOME%/buildtools/bin/ntrmln.bat rmf %filename%.exe.manifest
goto end
